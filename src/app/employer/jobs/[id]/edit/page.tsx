...
export default function EditJobPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (jobError) {
        setMsg(jobError.message);
        setLoading(false);
        return;
      }

      // Проверяем, что вакансия принадлежит одной из компаний пользователя
      if (!list.find((c) => c.id === job.company_id)) {
        setMsg("У вас нет доступа к этой вакансии.");
        setLoading(false);
        return;
      }
      ...
      setCity(job.city || "");
      setFormat(job.work_format || "office");
      setEmploymentType(job.employment_type || "full-time");
      setExperience(job.experience_level || "middle");
      setEducationLevel(job.education_level || "");
      setResponsibilities(job.responsibilities || "");
      
      // Разбираем benefits обратно
      const benefitsStr = job.benefits || "";
      const parts = benefitsStr.split("\n\n");
      if (parts.length > 1 && parts[1].startsWith("Что предлагает компания:")) {
        setBenefits(parts[0]);
      } else {
        setBenefits(benefitsStr);
      }

      setSalaryNegotiable(job.salary_negotiable || false);
      if (!job.salary_negotiable) {
        if (job.salary_from) setSalaryFromText(fmt(String(job.salary_from)));
        if (job.salary_to) setSalaryToText(fmt(String(job.salary_to)));
      }
  ...
  async function onUpdate() {
    setMsg(null);
    if (!title.trim()) {
      setMsg("Введите название вакансии.");
      return;
    }
    if (!companyId) {
      setMsg("Выберите компанию.");
      return;
    }
    if (!description.trim()) {
      setMsg("Введите описание вакансии.");
      return;
    }
    if (!city.trim()) {
      setMsg("Введите город.");
      return;
    }
  ...
  const inputCls =
    "w-full rounded-2xl bg-black/20 border border-white/10 px-4 py-3 outline-none focus:border-violet-500/50 transition";
    ...
    <div className="min-h-screen bg-[#0b1220] text-white p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-semibold">Редактировать вакансию</h1>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {tagsText
                      ...
                      .map((tag) => (
                        <span
                          key={tag}
                          className="text-xs px-2 py-1 rounded-full bg-violet-600/20 text-violet-300 border border-violet-500/20"
                        >
                          {tag}
                        </span>
                      ))}
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${
                    isActive ? "bg-violet-600" : "bg-white/20"
                  }`}
                >
                  <div
                ...
                <span className="text-sm text-white/70">
                  {isActive ? "Вакансия активна" : "Снять с публикации"}
                </span>
              </label>
            </div>
          </div>
